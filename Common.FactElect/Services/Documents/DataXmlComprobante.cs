...
public class DataXmlComprobante
{
    ...
    public void GetInfoTributaria(ComprobanteElectronico ce, XmlNode nodeInfoTrib)
    {
    ...
    public void GetInfoNotaCredito(string codDoc, ComprobanteElectronico ce, XmlNode nodeInfoNotaCredito)
    {
        ...
        infoFac.fechaEmision = nodeInfoNotaCredito.SelectSingleNode("fechaEmision").InnerText;
        infoFac.dirEstablecimiento = nodeInfoNotaCredito.SelectSingleNode("dirEstablecimiento") == null
            ? ""
        ...
        infoFac.razonSocialComprador = nodeInfoNotaCredito.SelectSingleNode("razonSocialComprador").InnerText;
        infoFac.identificacionComprador = nodeInfoNotaCredito.SelectSingleNode("identificacionComprador").InnerText;
        infoFac.contribuyenteEspecial = nodeInfoNotaCredito.SelectSingleNode("contribuyenteEspecial") == null
            ? ""
            : nodeInfoNotaCredito.SelectSingleNode("contribuyenteEspecial").InnerText;
        infoFac.obligadoContabilidad = nodeInfoNotaCredito.SelectSingleNode("obligadoContabilidad") == null
            ? ""
            : nodeInfoNotaCredito.SelectSingleNode("obligadoContabilidad").InnerText;
        infoFac.rise = nodeInfoNotaCredito.SelectSingleNode("rise") == null
            ? ""
            : nodeInfoNotaCredito.SelectSingleNode("rise").InnerText;
        infoFac.codDocModificado = nodeInfoNotaCredito.SelectSingleNode("codDocModificado").InnerText;
        infoFac.numDocModificado = nodeInfoNotaCredito.SelectSingleNode("numDocModificado").InnerText;
        infoFac.fechaEmisionDocSustento = nodeInfoNotaCredito.SelectSingleNode("fechaEmisionDocSustento") == null
            ? ""
            : nodeInfoNotaCredito.SelectSingleNode("fechaEmisionDocSustento").InnerText;
        infoFac.totalSinImpuestos = nodeInfoNotaCredito.SelectSingleNode("totalSinImpuestos").InnerText;
        infoFac.valorModificacion = nodeInfoNotaCredito.SelectSingleNode("valorModificacion") == null
            ? ""
            : nodeInfoNotaCredito.SelectSingleNode("valorModificacion").InnerText;
        infoFac.moneda = nodeInfoNotaCredito.SelectSingleNode("moneda") == null
            ? ""
            : nodeInfoNotaCredito.SelectSingleNode("moneda").InnerText;
        infoFac.motivo = nodeInfoNotaCredito.SelectSingleNode("motivo") == null
            ? ""
    ...
    private void GetInfoFactura(string codDoc, ComprobanteElectronico ce, XmlNode nodeInfoFactura)
    {
        ...
        infoFac.tipoIdentificacionComprador = nodeInfoFactura.SelectSingleNode("tipoIdentificacionComprador").InnerText;
    ...
    private void GetInfoCompRetencion(string versionComp, ComprobanteElectronico ce, XmlNode nodeInfoCompRetencion)
    {
        if (nodeInfoCompRetencion != null)
        {
            ...
            infoRet.fechaEmision = nodeInfoCompRetencion.SelectSingleNode("fechaEmision").InnerText;
            infoRet.dirEstablecimiento = nodeInfoCompRetencion.SelectSingleNode("dirEstablecimiento") == null
                ? ""
                : nodeInfoCompRetencion.SelectSingleNode("dirEstablecimiento").InnerText;
            infoRet.contribuyenteEspecial = nodeInfoCompRetencion.SelectSingleNode("contribuyenteEspecial") == null
                ? ""
                : nodeInfoCompRetencion.SelectSingleNode("contribuyenteEspecial").InnerText;
            infoRet.obligadoContabilidad = nodeInfoCompRetencion.SelectSingleNode("obligadoContabilidad").InnerText;
            infoRet.tipoIdentificacionSujetoRetenido =
                nodeInfoCompRetencion.SelectSingleNode("tipoIdentificacionSujetoRetenido").InnerText;
            infoRet.razonSocialSujetoRetenido =
                nodeInfoCompRetencion.SelectSingleNode("razonSocialSujetoRetenido").InnerText;
            infoRet.identificacionSujetoRetenido =
                nodeInfoCompRetencion.SelectSingleNode("identificacionSujetoRetenido").InnerText;
            infoRet.periodoFiscal = nodeInfoCompRetencion.SelectSingleNode("periodoFiscal") == null
                ? ""
    ...
    private void GetImpuestoRetencion(ComprobanteElectronico comp, XmlNode impuestosRet)
    {
        ...
        foreach (XmlElement taxes in impuestosRet)
        {
            ...
            tax.codigo = taxes.GetElementsByTagName("codigo")[0].InnerText;
            tax.codigoRetencion = taxes.GetElementsByTagName("codigoRetencion")[0].InnerText;
            tax.baseImponible = taxes.GetElementsByTagName("baseImponible")[0].InnerText;
            tax.porcentajeRetener = taxes.GetElementsByTagName("porcentajeRetener")[0].InnerText;
            tax.valorRetenido = taxes.GetElementsByTagName("valorRetenido")[0].InnerText;
            tax.codDocSustento = taxes.GetElementsByTagName("codDocSustento")[0].InnerText;
            tax.numDocSustento = taxes.GetElementsByTagName("numDocSustento")[0].InnerText;
            tax.fechaEmisionDocSustento = taxes.GetElementsByTagName("fechaEmisionDocSustento")[0].InnerText;
            impuestos.Add(tax);
        }
    ...
    private void GetDocSustento(ComprobanteElectronico ce, XmlNode nodeDocsSustento)
    {
        ...
        foreach (XmlElement item in nodeDocsSustento)
        {
            ...
            docSustento.codSustento = item.GetElementsByTagName("codSustento")[0].InnerText;
            docSustento.codDocSustento = item.GetElementsByTagName("codDocSustento")[0].InnerText;
            docSustento.numDocSustento = item.GetElementsByTagName("numDocSustento")[0].InnerText;
            docSustento.fechaEmisionDocSustento = item.GetElementsByTagName("fechaEmisionDocSustento")[0].InnerText;
            docSustento.numAutDocSustento = item.SelectSingleNode("numAutDocSustento") == null
                ? ""
                : item.GetElementsByTagName("numAutDocSustento")[0].InnerText;
            docSustento.pagoLocExt = item.GetElementsByTagName("pagoLocExt")[0].InnerText;
            docSustento.tipoRegi = item.SelectSingleNode("tipoRegi") == null
                ? ""
                : item.GetElementsByTagName("tipoRegi")[0].InnerText;
            docSustento.paisEfecPago = item.SelectSingleNode("paisEfecPago") == null
                ? ""
                : item.GetElementsByTagName("paisEfecPago")[0].InnerText;
            docSustento.aplicConvDobTrib = item.SelectSingleNode("aplicConvDobTrib") == null
                ? ""
                : item.GetElementsByTagName("aplicConvDobTrib")[0].InnerText;
            docSustento.pagExtSujRetNorLeg = item.SelectSingleNode("pagExtSujRetNorLeg") == null
                ? ""
                : item.GetElementsByTagName("pagExtSujRetNorLeg")[0].InnerText;
            docSustento.pagoRegFis = item.SelectSingleNode("pagoRegFis") == null
                ? ""
                : item.GetElementsByTagName("pagoRegFis")[0].InnerText;
            docSustento.totalComprobantesReembolso = item.SelectSingleNode("totalComprobantesReembolso") == null
                ? ""
                : item.GetElementsByTagName("totalComprobantesReembolso")[0].InnerText;
            docSustento.totalBaseImponibleReembolso = item.SelectSingleNode("totalBaseImponibleReembolso") == null
                ? ""
                : item.GetElementsByTagName("totalBaseImponibleReembolso")[0].InnerText;
            docSustento.totalImpuestoReembolso = item.SelectSingleNode("totalImpuestoReembolso") == null
                ? ""
                : item.GetElementsByTagName("totalImpuestoReembolso")[0].InnerText;
            docSustento.totalSinImpuestos = item.GetElementsByTagName("totalSinImpuestos")[0].InnerText;
    ...
    {
        var impuestos = new List<ImpuestoDocSustento>();
        foreach (XmlElement item in nodeImpuestos)
        {
            var impuesto = new ImpuestoDocSustento();
            impuesto.codImpuestoDocSustento = item.GetElementsByTagName("codImpuestoDocSustento")[0].InnerText;
            impuesto.codigoPorcentaje = item.GetElementsByTagName("codigoPorcentaje")[0].InnerText;
            impuesto.baseImponible = item.GetElementsByTagName("baseImponible")[0].InnerText;
            impuesto.tarifa = item.GetElementsByTagName("tarifa")[0].InnerText;
            impuesto.valorImpuesto = item.GetElementsByTagName("valorImpuesto")[0].InnerText;
    ...
    private void GetRetenciones(DocSustento doc, XmlNode nodeRetenciones, ComprobanteElectronico ce)
    {
        ...
        foreach (XmlElement item in nodeRetenciones)
        {
            ...
            retencion.codigo = item.GetElementsByTagName("codigo")[0].InnerText;
            retencion.codigoRetencion = item.GetElementsByTagName("codigoRetencion")[0].InnerText;
            retencion.baseImponible = item.GetElementsByTagName("baseImponible")[0].InnerText;
            retencion.porcentajeRetener = item.GetElementsByTagName("porcentajeRetener")[0].InnerText;
            retencion.valorRetenido = item.GetElementsByTagName("valorRetenido")[0].InnerText;
            retenciones.Add(retencion);

            var ir = new ImpuestoRetencion();
            ir.codigo = item.GetElementsByTagName("codigo")[0].InnerText;
            ir.codigoRetencion = item.GetElementsByTagName("codigoRetencion")[0].InnerText;
            ir.baseImponible = item.GetElementsByTagName("baseImponible")[0].InnerText;
            ir.porcentajeRetener = item.GetElementsByTagName("porcentajeRetener")[0].InnerText;
            ir.valorRetenido = item.GetElementsByTagName("valorRetenido")[0].InnerText;
            ir.codDocSustento = doc.codDocSustento;
            ir.numDocSustento = doc.numDocSustento;
            ir.fechaEmisionDocSustento = doc.fechaEmisionDocSustento;
            retToView.Add(ir);
        }
    ...
    private void GetReembolsos(DocSustento doc, XmlNode nodeReemb)
    {
        ...
        foreach (XmlElement item in nodeReemb)
        {
            ...
            reembolso.numeroAutorizacionDocReemb = item.GetElementsByTagName("numeroAutorizacionDocReemb")[0].InnerText;
    ...
    private void GetImpuestosReembolsos(ReembolsoDetalle reemb, XmlNode nodeImpReemb)
    {
        var impuestos = new List<DetalleImpuesto>();
        foreach (XmlElement item in nodeImpReemb)
        {
            var impuesto = new DetalleImpuesto();
            impuesto.codigo = item.GetElementsByTagName("codigo")[0].InnerText;
            impuesto.codigoPorcentaje = item.GetElementsByTagName("codigoPorcentaje")[0].InnerText;
            impuesto.tarifa = item.GetElementsByTagName("tarifa")[0].InnerText;
            impuesto.baseImponibleReembolso = item.GetElementsByTagName("baseImponibleReembolso")[0].InnerText;
            impuesto.impuestoReembolso = item.GetElementsByTagName("impuestoReembolso")[0].InnerText;
            impuestos.Add(impuesto);
        }

        reemb.CreateTax(impuestos);
    }
